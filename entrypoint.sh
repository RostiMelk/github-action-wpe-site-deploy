...
# Deploy via SSH
rsync -aHAXxv --numeric-ids --progress --rsh="ssh -v -p 22 -i ${WPE_SSHG_KEY_PRIVATE_PATH} StrictHostKeyChecking=no -T -o Compression=no -x" --exclude=".*" $SRC_PATH "$WPE_DESTINATION"
